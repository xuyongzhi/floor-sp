...
def primitive_detector_eval(all_dets,
    ...
    gt_records = [[{'bbox': list(), 'det': list(), 'mask': list(), 'edge_dir': list()} for _ in range(len(vocabulary))]
                  for _ in
                  range(len(roidb))]
...
def compute_mask_iou(gt_mask, mask_pred, bbox_pred, MAX_SIZE=255):
    ...
    cropped_pred = mask_pred[int(bbox_pred[0]):int(bbox_pred[2]) + 1, int(bbox_pred[1]):int(bbox_pred[3]) + 1].astype(
        bool)
    resized_gt = cv2.resize(gt_mask,
                            (int(bbox_pred[3]) - int(bbox_pred[1]) + 1, int(bbox_pred[2]) - int(bbox_pred[0]) + 1))
